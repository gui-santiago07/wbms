...
export const useProductionDataPolling = (clientLineKey: string | null, enabled: boolean = true) => {
  const { loadProductionData, loadDeviceStatus, loadApiProductionStatus, setupData } = useProductionStore();
  ...
  }, [enabled, clientLineKey, setupData?.line, loadProductionData, loadDeviceStatus, loadApiProductionStatus]);
